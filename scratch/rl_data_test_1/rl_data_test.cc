...
using namespace ns3;
NS_LOG_COMPONENT_DEFINE ("Rana-HO-test1-online");
...
int main(int argc, char *argv[])
{
  ...
  std::string fadingName = "./src/lte/model/fading-traces/fading_trace_EVA_60kmph.fad";
  std::string traceFile, logFile;
  
  // Switch for building and fading
  bool enableFading = true;
  bool useThreeGpp = true;
  std::string pathLossModel = "ns3::LogDistancePropagationLossModel"; 
  if(useThreeGpp){pathLossModel = "ns3::ThreeGppRmaPropagationLossModel";}
  ...
  */
  ...
  uint16_t numBS = (numEnbs - 1) / 3 + 1;
  double x_BS[numBS] = {144, 594 * 4, 609 * 4, 200 * 3, 1500};
  double y_BS[numBS] = {266, 300 * 3, 1176 * 2.5, 981 * 2.5, 1500};
  ...
  for (uint16_t i = 0; i < numBS; i++) 
  {
    ...
    {
      positionAllocEnbs->Add(Vector(x_BS[i], y_BS[i], enbHeight));
    }
  }
  ...
  if (enableFading)
{
  ...
  if(pathLossModel == "ns3::ThreeGppRmaPropagationLossModel")
  { 
    lteHelper->SetAttribute ("PathlossModel", StringValue (pathLossModel));
    ...
    lteHelper->SetPathlossModelAttribute ("AvgStreetWidth", DoubleValue (5));
    std::cout << "Pathloss model 3GPP Rma setup complete." << std::endl;
  }
  ...
  // NetDeviceContainer enbDevs = lteHelper->InstallEnbDevice (eNbs.Get(numEnbs-1)); // For default isotropic setting
  // NetDeviceContainer enbDevs = lteHelper->InstallEnbDevice (eNbs);
  // std::cout <<"BS good \n";

  double AntennaOrientation[numEnbs] = {125, 226, 15, 192, 71, 266, 320, 111, 200, 324, 97, 187, 90, 210, 330}; //, 268, 105, 230, 159, 315}; // For cosine antenna orientation for each cell as above
  // double AntennaOrientation[numEnbs] = {125, 226, 357, 192, 71, 206, 320, 111, 239, 324, 97, 13}; //, 268, 105, 230, 159, 315}; // For cosine antenna orientation for each cell as above

  // Install eNB devices
  for (uint16_t i = 0; i < numEnbs; i++)
  {
    lteHelper->SetEnbAntennaModelType ("ns3::CosineAntennaModel");
    lteHelper->SetEnbAntennaModelAttribute ("Orientation", DoubleValue (AntennaOrientation[i]-90)); // Adjust offset because ns-3 use X-axis as 0
    // Same settings as Shuja's previous work
    lteHelper->SetEnbAntennaModelAttribute ("MaxGain", DoubleValue (15.0));  
    lteHelper->SetEnbAntennaModelAttribute ("HorizontalBeamwidth", DoubleValue (85.0));
